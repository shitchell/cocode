...
var FirepadUserList = (function() {
  function FirepadUserList(ref, place, userId, displayName) {
    ...
    console.log("display name passed in: " + displayName);
    ...
    this.onNameChange = function(oldName, newNam) {};
    this.onUserConnected = function(user) {};
  ...
  FirepadUserList.prototype.makeHeading_ = function() {
    ...
    var headerCounterSpan = document.getElementById('user-count');
    this.firebaseOn_(this.ref_, 'value', function(usersSnapshot) {
      setTextContent(counterSpan, "" + usersSnapshot.numChildren());
      setTextContent(headerCounterSpan, "" + usersSnapshot.numChildren());
    });
  ...
  FirepadUserList.prototype.makeUserEntriesForOthers_ = function() {
    ...
    function updateChild(userSnapshot, prevChildName) {
      ...
      if (userId.startsWith("-")) {
      	console.log("Ignoring snapshot: ", userSnapshot);
      	return;
      }
      var div = userId2Element[userId];
      if (div) {
      	// Alert about the name change
      	var oldName = div.children[1].innerText
      	var newName = userSnapshot.child('name').val();
      	self.onNameChange(oldName, newName);
